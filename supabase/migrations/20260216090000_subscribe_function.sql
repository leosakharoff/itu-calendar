-- RPC function to subscribe to a shared calendar
-- Uses SECURITY DEFINER to bypass RLS and read the source user's data
...
BEGIN
  ...
  -- Copy all events
  INSERT INTO events (course_id, title, date, type, notes, user_id)
  SELECT v_new_course_id, e.title, e.date, e.type, e.notes, v_uid
  FROM events e
